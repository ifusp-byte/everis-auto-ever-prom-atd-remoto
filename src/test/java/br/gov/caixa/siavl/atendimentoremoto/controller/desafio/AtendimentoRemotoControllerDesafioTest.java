...
class AtendimentoRemotoControllerDesafioTest extends ControllerTest {
	...
	void desafioCriarPF() throws StreamReadException, DatabindException, IOException {
		...
		CriaDesafioInputDTO criaDesafioInputDTO1 = mapper.readValue(
				new ClassPathResource("/desafio/1criaDesafioInputDTO.json").getFile(), CriaDesafioInputDTO.class);
		ResponseEntity<Object> response1 = restTemplate.exchange(BASE_URL, HttpMethod.POST,
				newRequestEntity(criaDesafioInputDTO1), Object.class);
		Assertions.assertEquals(HttpStatus.OK, response1.getStatusCode());
	}

	void desafioCriarPJ() throws StreamReadException, DatabindException, IOException {
		String BASE_URL = atdremotoUrl + "/desafio-criar/" + "10020030088";
		CriaDesafioInputDTO criaDesafioInputDTO2 = mapper.readValue(
				new ClassPathResource("/desafio/2criaDesafioInputDTO.json").getFile(), CriaDesafioInputDTO.class);
		ResponseEntity<Object> response2 = restTemplate.exchange(BASE_URL, HttpMethod.POST,
				newRequestEntity(criaDesafioInputDTO2), Object.class);
	...
	void desafioResponderPF() throws StreamReadException, DatabindException, IOException {
		...
		RespondeDesafioInputDTO respondeDesafioInputDTO = mapper.readValue(
				new ClassPathResource("/desafio/1respondeDesafioInputDTO.json").getFile(),
				RespondeDesafioInputDTO.class);
		ResponseEntity<Object> response1 = restTemplate.exchange(BASE_URL, HttpMethod.POST,
				newRequestEntity(respondeDesafioInputDTO), Object.class);
		Assertions.assertEquals(HttpStatus.OK, response1.getStatusCode());
	}
