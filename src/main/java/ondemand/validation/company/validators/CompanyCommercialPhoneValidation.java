...
public class CompanyCommercialPhoneValidation implements CompanyValidator {
    ...
    public CompanyCommercialPhoneValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validate(CompanyDto company) {
        ...
        if (!Pattern.compile(REGEX_COMPANY_COMMERCIAL_PHONE).matcher(commercialPhone).matches()) {
            ...
            String violationMessage = Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_TELEFONE_COMERCIAL, messageSource);
