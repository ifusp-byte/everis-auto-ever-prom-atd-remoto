...
public class CompanyHeaderAcronymStateValidation implements CompanyHeaderValidator {
    ...
    public CompanyHeaderAcronymStateValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validateHeader(CompanyDto company) {
        ...
        if (!acronymState.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_SIGLA_ESTADO, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(companyValidatorService.newViolation(statusReturn, Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_SIGLA_ESTADO, messageSource), acronymState));
        }
