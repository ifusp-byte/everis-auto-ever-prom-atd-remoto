...
@Component
public class CompanyHeaderTaxpayerRegistrationValidation implements CompanyHeaderValidator {
    private final CompanyValidatorService companyValidatorService;
    private final MessageSource messageSource;

    public CompanyHeaderTaxpayerRegistrationValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validateHeader(CompanyDto company) {
        ...
        if (!taxpayerRegistration.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CNPJ, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(companyValidatorService.newViolation(statusReturn, Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CNPJ, messageSource), taxpayerRegistration));
        }
