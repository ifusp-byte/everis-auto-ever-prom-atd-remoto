...
public class CompanyHeaderEmployeeValidation implements CompanyHeaderValidator {
    ...
    public CompanyHeaderEmployeeValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validateHeader(CompanyDto company) {
        ...
        if (!employee.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_FUNCIONARIO, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(companyValidatorService.newViolation(statusReturn, Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_FUNCIONARIO, messageSource), employee));
        }
