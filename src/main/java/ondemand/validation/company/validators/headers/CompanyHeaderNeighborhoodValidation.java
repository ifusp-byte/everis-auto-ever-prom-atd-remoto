...
public class CompanyHeaderNeighborhoodValidation implements CompanyHeaderValidator {
    ...
    public CompanyHeaderNeighborhoodValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validateHeader(CompanyDto company) {
        ...
        if (!neighborhood.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_BAIRRO, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(companyValidatorService.newViolation(statusReturn, Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_BAIRRO, messageSource), neighborhood));
        }
