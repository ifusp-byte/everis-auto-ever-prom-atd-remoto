...
@Component
public class CompanyCellphoneValidation implements CompanyValidator {
    private final CompanyValidatorService companyValidatorService;
    private final MessageSource messageSource;

    public CompanyCellphoneValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validate(CompanyDto company) {
        ...
        if (!Pattern.compile(REGEX_COMPANY_CELLPHONE).matcher(cellphone).matches()) {
            boolean statusReturn = true;
            String violationMessage = Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_TELEFONE_CELULAR, messageSource);
            return Optional.of(companyValidatorService.newViolation(statusReturn, violationMessage, cellphone));
        }
