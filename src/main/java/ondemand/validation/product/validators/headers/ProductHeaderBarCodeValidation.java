...
public class ProductHeaderBarCodeValidation implements ProductHeaderValidator {
    ...
    public ProductHeaderBarCodeValidation(MessageSource messageSource, @Lazy ProductValidatorService productValidatorService) {
    ...
    public Optional<ViolationDto> validateHeader(ProductDto product) {
        ...
        if (!barCode.equals(Util.getMessageSource(ProductConstants.VALIDATION_PRODUCT_HEADER_CODIGO_BARRAS, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(productValidatorService.newViolation(statusReturn, Util.getMessageSource(ProductConstants.VALIDATION_PRODUCT_HEADER_CODIGO_BARRAS, messageSource), barCode));
        }
