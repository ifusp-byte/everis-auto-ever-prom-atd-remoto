...
public class SchedulerJobProcessBankSlipThread extends ThreadsDefault {
    ...
    public SchedulerJobProcessBankSlipThread(AwsSecretsDto awsSecretsDto, MessageSource messageSource, ServiceAwsSqs serviceAwsSqs,
            UsuarioService userService, TokenInterceptorService tokenInterceptorService, AccountsReceivableByContractService accountsReceivableByContractService) {
    ...
    public void run() {
        ...
        List<Message> queueMessages = serviceAwsSqs.returnMessages(awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueSchedulerJobBankSlip());
        ...
        for (Message message : queueMessages) {
            try {
                ...
                LOG.error(Util.formatExceptionMessage(SCHEDULER_JOB_BANKSLIP_THREAD_ERROR_PROCESS, messageSource, schedulerJobDto.getContractId().toString()), e.getMessage(), e);
            }
        }
    }
    ...
    private void deleteMessage(Message message) {
        serviceAwsSqs.getClienteSQS().deleteMessage(
                awsSecretsDto.getUrlFilasSqs() 
                + awsSecretsDto.getQueueSchedulerJobBankSlip(), 
