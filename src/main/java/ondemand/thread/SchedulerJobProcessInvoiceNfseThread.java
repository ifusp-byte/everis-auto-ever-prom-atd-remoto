...
public class SchedulerJobProcessInvoiceNfseThread extends ThreadsDefault {
    ...
    public SchedulerJobProcessInvoiceNfseThread(AwsSecretsDto awsSecretsDto, MessageSource messageSource, ServiceAwsSqs serviceAwsSqs,
            UsuarioService userService, TokenInterceptorService tokenInterceptorService, InvoiceNfseByContractService invoiceNfseByContractService) {
    ...
    public void run() {
        ...
        List<Message> queueMessages = serviceAwsSqs.returnMessages(awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueSchedulerJobInvoiceNfse());
        ...
        for (Message message : queueMessages) {
            try {
                ...
                LOG.error(Util.formatExceptionMessage(SCHEDULER_JOB_INVOICE_NFSE_THREAD_ERROR_PROCESS, messageSource, schedulerJobDto.getContractId().toString()), e.getMessage(), e);
            }
        }
    }
    ...
    private void deleteMessage(Message message) {
        serviceAwsSqs.getClienteSQS().deleteMessage(
                ...
                + awsSecretsDto.getQueueSchedulerJobInvoiceNfse(), 
