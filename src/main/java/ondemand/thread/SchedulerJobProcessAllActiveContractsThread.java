...
public class SchedulerJobProcessAllActiveContractsThread extends ThreadsDefault {
    ...
    public SchedulerJobProcessAllActiveContractsThread(AwsSecretsDto awsSecretsDto, MessageSource messageSource, ServiceAwsSqs serviceAwsSqs,
            UsuarioService userService, TokenInterceptorService tokenInterceptorService, ContractBySchedulerJobService contractBySchedulerJobService) {
    ...
    public void run() {
        ...
        List<Message> queueMessages = serviceAwsSqs.returnMessages(awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueSchedulerJob());
        ...
        for (Message message : queueMessages) {
            try {
                ...
                LOG.error(Util.formatExceptionMessage(SCHEDULER_JOB_PROCESS_THREAD_PROCESS, messageSource, schedulerJobDto.getUserId().toString()), e.getMessage(), e);
            }
        }
    }
    ...
    private void deleteMessage(Message message) {
        serviceAwsSqs.getClienteSQS().deleteMessage(
                ...
                + awsSecretsDto.getQueueSchedulerJob(), 
