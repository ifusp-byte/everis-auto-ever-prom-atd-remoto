...
public class BankBatchReturnFileThread extends ThreadsDefault {
	...
	public BankBatchReturnFileThread(AwsSecretsDto awsSecretsDto, MessageSource messageSource,
			ServiceAwsSqs serviceAwsSqs, UsuarioService userService, TokenInterceptorService tokenInterceptorService,
			BankBatchReturnFileService bankBatchReturnFileService) {
	...
	public void run() {
		...
		List<Message> queueMessages = serviceAwsSqs
				.returnMessages(awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueBankBatchReturnFile());
		...
		for (Message message : queueMessages) {
			try {
				...
				BankBatchReturnFileImported bankBatchReturnFileImported = bankBatchReturnFileService
						.processBankBatchReturnFile(jsonReturnFile);

				bankBatchReturnFileService.afterProcessingReturnFileSendEmailAndDeleteFileAws(jsonReturnFile,
						bankBatchReturnFileImported);
				serviceAwsSqs.getClienteSQS().deleteMessage(
						awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueBankBatchReturnFile(),
						message.getReceiptHandle());

			} catch (Exception e) {
				LOG.error(
						Util.formatExceptionMessage(ERROR_BANK_BATCH_RETURN_FILE_PROCESS, messageSource,
								String.valueOf(jsonReturnFile.getLong(USER_ID)), jsonReturnFile.getString(FILE_NAME)),
						e.getMessage(), e);
			}
		}
	}
