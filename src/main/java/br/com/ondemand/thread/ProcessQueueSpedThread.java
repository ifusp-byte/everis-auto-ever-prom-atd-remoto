...
public class ProcessQueueSpedThread extends ThreadsDefault {
	...
	public ProcessQueueSpedThread(AwsSecretsDto awsSecretsDto, ServiceAwsSqs serviceAwsSqs, UsuarioService userService,
			TokenInterceptorService tokenInterceptorService, SpedService spedService) {
	...
	public void run() {
		...
		List<Message> queueMessages = serviceAwsSqs
				.returnMessages(awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueSped());
		...
		for (Message message : queueMessages) {
			try {
				...
				serviceAwsSqs.getClienteSQS().deleteMessage(
						awsSecretsDto.getUrlFilasSqs() + awsSecretsDto.getQueueSped(), message.getReceiptHandle());

			} catch (Exception e) {
				LOG.error("Error processing ProcessQueueSpedThread failed: {} | Reason: {}", message.getBody(),
						e.getMessage(), e);
			}
		}
	}
