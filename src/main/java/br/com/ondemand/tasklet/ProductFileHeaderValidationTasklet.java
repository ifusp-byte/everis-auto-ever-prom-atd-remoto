	...
	public RepeatStatus execute(StepContribution stepContribution, ChunkContext chunkContext) throws Exception {
		...
		if (lastColmNum != SIX) {
			chunkContext.getStepContext().getStepExecution().getExecutionContext()
					.put(CONTEXT_PRODUCT_FILE_HEADER_VALID, true);
			chunkContext.getStepContext().getStepExecution().getExecutionContext()
					.put(CONTEXT_PRODUCT_FILE_HEADER_VIOLATIONS, StringUtils.EMPTY);
		...
		violationStatus = violations.stream()
				.anyMatch(violation -> Boolean.TRUE.equals(violation.getViolationStatus()));
		...
		chunkContext.getStepContext().getStepExecution().getExecutionContext().put(CONTEXT_PRODUCT_FILE_HEADER_VALID,
				violationStatus);
		chunkContext.getStepContext().getStepExecution().getExecutionContext()
				.put(CONTEXT_PRODUCT_FILE_HEADER_VIOLATIONS, headerViolations);
		return RepeatStatus.FINISHED;
	}

	private void mapAllProductCellValues(int lastColmNum, HashMap<String, String> allProductCellValues, HSSFRow hssfRow,
			XSSFRow xssfRow) {
		AtomicInteger colm = new AtomicInteger(ZERO_NUMBER);

		if (hssfRow != null) {
			for (int colmNum = ZERO_NUMBER; colmNum < lastColmNum; colmNum++) {
				allProductCellValues.put(ProductDto.class.getDeclaredFields()[colmNum].getName(),
						hssfRow.getCell(colm.getAndIncrement()).getStringCellValue());
			}
		}
		...
		if (xssfRow != null) {
			for (int colmNum = ZERO_NUMBER; colmNum < lastColmNum; colmNum++) {
				allProductCellValues.put(ProductDto.class.getDeclaredFields()[colmNum].getName(),
						xssfRow.getCell(colm.getAndIncrement()).getStringCellValue());
			}
		}
	}
}
