...
public class CompanyCellphoneValidation implements CompanyValidator {
	...
	public CompanyCellphoneValidation(@Lazy CompanyValidatorService companyValidatorService,
	...
	public Optional<ViolationDto> validate(CompanyDto company) {
		...
		if (!Pattern.compile(REGEX_COMPANY_CELLPHONE).matcher(cellphone).matches()) {
			...
			String violationMessage = Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_TELEFONE_CELULAR,
					messageSource);
