...
public class CompanyHeaderAreaCodeValidation implements CompanyHeaderValidator {
	...
	public CompanyHeaderAreaCodeValidation(@Lazy CompanyValidatorService companyValidatorService,
	...
	public Optional<ViolationDto> validateHeader(CompanyDto company) {
		...
		if (!cellphoneAreaCode
				.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_DDD, messageSource))
				|| !commercialAreaCode
						.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_DDD, messageSource))) {
			boolean statusReturn = true;
			return Optional.of(companyValidatorService.newViolation(statusReturn,
					Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_DDD, messageSource)));
		}
