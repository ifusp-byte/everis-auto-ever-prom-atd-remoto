...
public class CompanyHeaderTaxpayerRegistrationValidation implements CompanyHeaderValidator {
	...
	public CompanyHeaderTaxpayerRegistrationValidation(@Lazy CompanyValidatorService companyValidatorService,
	...
	public Optional<ViolationDto> validateHeader(CompanyDto company) {
		...
		if (!taxpayerRegistration
				.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CNPJ, messageSource))) {
			boolean statusReturn = true;
			return Optional.of(companyValidatorService.newViolation(statusReturn,
					Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CNPJ, messageSource),
