...
public class CompanyHeaderCellphoneValidation implements CompanyHeaderValidator {
	...
	public CompanyHeaderCellphoneValidation(@Lazy CompanyValidatorService companyValidatorService,
	...
	public Optional<ViolationDto> validateHeader(CompanyDto company) {
		...
		if (!cellphone
				.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CELULAR, messageSource))) {
			boolean statusReturn = true;
			return Optional.of(companyValidatorService.newViolation(statusReturn,
					Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CELULAR, messageSource),
					cellphone));
		}
