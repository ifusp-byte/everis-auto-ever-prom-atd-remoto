...
public class CompanyHeaderCityValidation implements CompanyHeaderValidator {
	...
	public CompanyHeaderCityValidation(@Lazy CompanyValidatorService companyValidatorService,
	...
	public Optional<ViolationDto> validateHeader(CompanyDto company) {
		...
		if (!city.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CIDADE, messageSource))) {
			...
			return Optional.of(companyValidatorService.newViolation(statusReturn,
					Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_CIDADE, messageSource), city));
		}
