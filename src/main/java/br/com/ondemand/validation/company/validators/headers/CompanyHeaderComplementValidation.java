...
@Component
public class CompanyHeaderComplementValidation implements CompanyHeaderValidator {
    private final CompanyValidatorService companyValidatorService;
    private final MessageSource messageSource;

    public CompanyHeaderComplementValidation(@Lazy CompanyValidatorService companyValidatorService, MessageSource messageSource) {
    ...
    public Optional<ViolationDto> validateHeader(CompanyDto company) {
        String complement = company.getComplemento();
        if (!complement.equals(Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_COMPLEMENTO, messageSource))) {
            boolean statusReturn = true;
            return Optional.of(companyValidatorService.newViolation(statusReturn, Util.getMessageSource(CompanyConstants.VALIDATION_COMPANY_HEADER_COMPLEMENTO, messageSource), complement));
        }
