...
import main.java.br.com.ondemand.listener.ProductFileExtensionValidationListener;
import main.java.br.com.ondemand.listener.ProductFileHeaderValidationListener;
import main.java.br.com.ondemand.listener.ProductJobListener;
import main.java.br.com.ondemand.listener.ProductReaderListener;
import main.java.br.com.ondemand.tasklet.ProductFileExtensionValidationTasklet;
import main.java.br.com.ondemand.tasklet.ProductFileHeaderValidationTasklet;
...
public class ProductConfig {
	...
	public ProductConfig(JobBuilderFactory jobBuilderFactory, StepBuilderFactory stepBuilderFactory,
			@Lazy ProductValidatorService productValidatorService, MessageSource messageSource,
	...
	public Job productJob(Flow productFileExtensionValidationFlow, Flow productFileHeaderValidationFlow) {
		return jobBuilderFactory.get(JOB_PRODUCT).listener(new ProductJobListener(messageSource))
				.start(productFileExtensionValidationFlow).on(STATUS_COMPLETED).to(productFileHeaderValidationFlow)
				.from(productFileExtensionValidationFlow).on(STATUS_FAILED).end().end().build();
	}

	@Bean
	public Flow productFileExtensionValidationFlow() {
		return new FlowBuilder<Flow>(FLOW_PRODUCT_FILE_EXTENSION_VALIDATION)
				.start(productFileExtensionValidationStep(stepBuilderFactory)).build();
	}

	@Bean
	public Flow productFileHeaderValidationFlow(
			@Qualifier(QUALIFIER_PRODUCT_ITEM_READER) PoiItemReader<ProductDto> itemReader,
		...
		return new FlowBuilder<Flow>(FLOW_PRODUCT_FILE_HEADER_VALIDATION)
				.start(productFileHeaderValidationStep(stepBuilderFactory)).on(STATUS_COMPLETED)
				.to(productReadWriteStep(itemReader, itemWriter))
				.from(productFileHeaderValidationStep(stepBuilderFactory)).on(STATUS_FAILED).end().build();
	}

	@Bean
	public Step productReadWriteStep(@Qualifier(QUALIFIER_PRODUCT_ITEM_READER) PoiItemReader<ProductDto> itemReader,
			@Qualifier(QUALIFIER_PRODUCT_ITEM_WRITER) ItemWriter<ProductDto> itemWriter) {
		return stepBuilderFactory.get(STEP_PRODUCT_READ_WRITE).<ProductDto, ProductDto>chunk(ROW_PARTITIONING)
				.reader(itemReader).writer(itemWriter).listener(new ProductReaderListener(fileImportAuditService))
				.build();
	}
	...
	public Step productFileExtensionValidationStep(StepBuilderFactory stepBuilders) {
		return stepBuilders.get(STEP_PRODUCT_FILE_EXTENSION_VALIDATION)
				...
				.listener(new ProductFileExtensionValidationListener(fileImportAuditService, messageSource)).build();
	}
	...
	public Step productFileHeaderValidationStep(StepBuilderFactory stepBuilders) {
		return stepBuilders.get(STEP_PRODUCT_FILE_HEADER_VALIDATION)
				...
				.listener(new ProductFileHeaderValidationListener(fileImportAuditService, messageSource)).build();
	}
