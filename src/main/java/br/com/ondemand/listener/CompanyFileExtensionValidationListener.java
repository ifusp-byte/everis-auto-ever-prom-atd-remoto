...
public class CompanyFileExtensionValidationListener implements StepExecutionListener {
	...
	public CompanyFileExtensionValidationListener(FileImportAuditService fileImportAuditService,
	...
	public void beforeStep(StepExecution stepExecution) {
		Long idFile = Long.parseLong(
				String.valueOf(stepExecution.getJobParameters().getParameters().get(CONTEXT_COMPANY_ID_FILE)));
		fileImportAuditService.jobExecutionContextContinuousAudit(idFile, FileImportStatusEnum.IN_PROGRESS,
				LocalDateTime.now());
		String fileResource = String
				.valueOf(stepExecution.getJobParameters().getParameters().get(CONTEXT_COMPANY_FILE_RESOURCE));
	...
	public ExitStatus afterStep(StepExecution stepExecution) {
		Long idFile = Long.parseLong(
				String.valueOf(stepExecution.getJobParameters().getParameters().get(CONTEXT_COMPANY_ID_FILE)));
		Boolean companyFileExtensionValid = Boolean.parseBoolean(
				String.valueOf(stepExecution.getExecutionContext().get(CONTEXT_COMPANY_FILE_EXTENSION_VALID)));
		...
		if (Boolean.TRUE.equals(companyFileExtensionValid)) {
			...
			fileImportAuditService.jobExecutionContextContinuousAudit(idFile, FileImportStatusEnum.FAILED,
					Util.getMessageSource(CompanyConstants.VIOLATION_MESSAGE_FILE_EXTENSION, messageSource),
					LocalDateTime.now());
