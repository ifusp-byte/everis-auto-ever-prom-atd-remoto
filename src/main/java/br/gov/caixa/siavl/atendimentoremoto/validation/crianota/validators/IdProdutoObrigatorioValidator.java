...
public class IdProdutoObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String idProduto = String.valueOf(Optional.ofNullable(criaNotaInputDto.getIdProduto()).orElse(StringUtils.EMPTY));

		if (StringUtils.isBlank(idProduto)) {
			return Optional.of(violacaoNova("idProduto não informado", idProduto));
		}
