...
public interface ModeloNotaRepository extends JpaRepository<ModeloNotaNegocioFavorito, Long> {
	...
	@Query(value="SELECT A.NU_MODELO_NOTA_NEGOCIO, B.NU_CAMPO_MODELO_NOTA, B.NU_ORDEM_CAMPO_MODELO, C.NO_CAMPO_MODELO_NOTA, C.IC_CAMPO_DEFINIDO, C.IC_CAMPO_EDITAVEL, C.IC_CAMPO_OBRIGATORIO, C.DE_ESPACO_RESERVADO, C.IC_TIPO_DADO_CAMPO, C.IC_TIPO_ENTRADA_CAMPO, C.DE_CAMPO, C.QT_CARACTER_CAMPO, C.VR_INICIAL_CAMPO, C.CO_MASCARA_CAMPO, D.NU_CONTEUDO_CAMPO_MULTIPLO, D.DE_CONTEUDO_CAMPO_MULTIPLO FROM AVL.AVLTB005_MODELO_NOTA_NEGOCIO A, AVL.AVLTB018_CAMPO_MODELO B,  AVL.AVLTB017_CAMPO_MODELO_NOTA C, AVL.AVLTB046_CNDO_CAMPO_MULTIPLO D WHERE A.NU_MODELO_NOTA_NEGOCIO = B.NU_MODELO_NOTA_NEGOCIO AND  B.NU_CAMPO_MODELO_NOTA = C.NU_CAMPO_MODELO_NOTA AND C.NU_CAMPO_MODELO_NOTA = D.NU_CAMPO_MODELO_NOTA AND A.NU_MODELO_NOTA_NEGOCIO =?1", nativeQuery=true)
	List<Object[]> modeloNotaDinamico(Long numeroModeloNota);
