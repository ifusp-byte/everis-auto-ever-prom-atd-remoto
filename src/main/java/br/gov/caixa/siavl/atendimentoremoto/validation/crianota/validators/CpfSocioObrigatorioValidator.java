...
public class CpfSocioObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String cpfSocio = String.valueOf(Optional.ofNullable(criaNotaInputDto.getCpfSocio()).orElse(StringUtils.EMPTY));
		String tipoDocumento = String.valueOf(Optional.ofNullable(criaNotaInputDto.getTipoDocumento()).orElse(StringUtils.EMPTY));

		if (TIPO_DOCUMENTO_CNPJ.equalsIgnoreCase(tipoDocumento) && StringUtils.isBlank(cpfSocio)) {
			return Optional.of(violacaoNova("cpfSocio não informado", cpfSocio));
		}
