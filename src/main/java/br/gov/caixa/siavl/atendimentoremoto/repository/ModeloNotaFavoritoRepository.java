...
public interface ModeloNotaFavoritoRepository extends JpaRepository<ModeloNotaNegocioFavorito, Long> {
	...
	@Query("SELECT A.numeroModeloNota, B.numeroAcao, B.descricao, C.numeroTipoNota, C.nomeTipoNota FROM ModeloNotaNegocio A, AcaoProduto B, TipoNota C WHERE C.numeroTipoNota = A.numeroTipoNota AND A.numeroAcao = B.numeroAcao AND B.acaoProdutoAtivo = 1 AND A.modeloNotaAtivo = 1 AND A.situacaoModeloNota = 'P' AND A.publicoAlvo <> ?1 ORDER BY B.descricao ASC")
	List<Object[]> findModeloNota(Long publicoAlvo);
	
	@Query("SELECT A.numeroModeloNota, B.numeroAcao, B.descricao, C.dataEscolhaFavorito, D.numeroTipoNota, D.nomeTipoNota FROM ModeloNotaNegocio A, AcaoProduto B, ModeloNotaNegocioFavorito C, TipoNota D WHERE D.numeroTipoNota = A.numeroTipoNota AND A.numeroAcao = B.numeroAcao AND A.numeroModeloNota = C.numeroModeloNota AND B.acaoProdutoAtivo = 1 AND A.modeloNotaAtivo = 1 AND A.situacaoModeloNota = 'P' AND C.matriculaFavorito = ?1 AND A.publicoAlvo <> ?2 ORDER BY C.dataEscolhaFavorito DESC")
