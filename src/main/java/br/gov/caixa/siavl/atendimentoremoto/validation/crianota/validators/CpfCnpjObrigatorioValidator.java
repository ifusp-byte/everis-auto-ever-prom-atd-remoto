...
public class CpfCnpjObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String cpfCnpj = String.valueOf(Optional.ofNullable(criaNotaInputDto.getCpfCnpj()).orElse(StringUtils.EMPTY));

		if (StringUtils.isBlank(cpfCnpj)) {
			return Optional.of(violacaoNova("cpfCnpj não informado", cpfCnpj));
		}
