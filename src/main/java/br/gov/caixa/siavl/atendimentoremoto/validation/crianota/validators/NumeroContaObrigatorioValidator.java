...
public class NumeroContaObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String numeroConta = String.valueOf(Optional.ofNullable(criaNotaInputDto.getNumeroConta()).orElse(StringUtils.EMPTY));

		if (StringUtils.isBlank(numeroConta)) {
			return Optional.of(violacaoNova("numeroConta não informado", numeroConta));
		}
