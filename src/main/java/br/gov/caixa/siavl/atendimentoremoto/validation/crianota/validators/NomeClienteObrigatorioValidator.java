...
public class NomeClienteObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String nomeCliente = String.valueOf(Optional.ofNullable(criaNotaInputDto.getNomeCliente()).orElse(StringUtils.EMPTY));

		if (StringUtils.isBlank(nomeCliente)) {
			return Optional.of(violacaoNova("nomeCliente não informado", nomeCliente));
		}
