...
public class TokenMatriculaObrigatorioValidator implements CriaNotaObrigatorioValidator {
	...
	public Optional<ViolacaoDto> validateObrigatorio(CriaNotaInputDTO criaNotaInputDto) {
		...
		String matricula = Optional.ofNullable(criaNotaInputDto.getToken()).orElse(StringUtils.EMPTY);
		String matriculaValida = tokenUtils.getMatriculaFromToken(matricula);

		if (StringUtils.isBlank(matriculaValida)) {
			return Optional.of(violacaoNova("Token preferred_username inválida", matriculaValida));
		}
